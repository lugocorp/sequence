...
export default class RiverEvent extends EventView {
  ...
  river(hero: Hero): void {
    ...
    hero.wis++;
    hero.dex++;
    this.game.views.setViews([{image: hero.sprite, text: `${hero.name} became stronger, wiser, and faster by the river!`, actions: {
      'continue': () => {
        if (hero.lucky()) {
          this.game.progress();
        } else {
  ...
  consequence(hero: Hero): void {
    ...
    this.game.chain.futureEvent(new RetrieveEvent(this.game, hero), 5, () => !this.game.party.isFull);
