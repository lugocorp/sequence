...
export default class SuspiciousItemEvent extends EventView {
  getViews(): View[] {
    ...
    return [{
      ...
      actions: {
        'continue': () => this.game.views.setViews(this.game.party.canPickupItems ?
          Selectors.heroes(this.game.party.members.filter((h: Hero) => h.basket.hasSpace), (hero: Hero) => ({
            'select': () => {
              this.game.chain.futureEvent(
                new SpiritWantsItsStuffBackEvent(this.game, hero, item),
                5,
                () => this.game.party.contains(hero) && hero.basket.contains(item)
              );
