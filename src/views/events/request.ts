...
export default class RequestEvent extends EventView {
  getViews(): View[] {
    ...
    const spiritView: View = {
      ...
      actions: {
        'view party': () => this.game.views.setViews(Selectors.heroes(this.game.party.members, (hero: Hero) => ({
          'choose': () => {
            const result: boolean = this.playerSatisfiesRequest(hero, challenger);
            if (result) {
        ...
        `${this.coloredRate(
          EnumsHelper.hasSkill(hero.skills, challenger.skill) ? 100 : hero.stats.luck
        )} chance of success`))
      }
    };
    return [{
